...
- **Invoke the `feature-workflow` skill** for the map: the ordered steps and which skill or agent runs each.

## Worktree & multi-session hygiene

I run several long-lived Claude sessions in the same repo, each in its own git worktree, and I `/clear` rather than close them. To keep switching work seamless:

- **One worktree = one feature, and disposable.** Don't recycle a worktree for unrelated work — spin a fresh one (`git worktree add <path> -b <branch>`) and tear it down (`git worktree remove`) once its branch merges.
- **Re-ground before starting new work.** A `/clear` resets the conversation, not the git reality. At the start of new work, check where you are — worktree, branch, clean/dirty — and **don't assume the branch**. When registered, the `session-orient` SessionStart hook injects this automatically; run the `/reground` skill for the full picture plus a verdict on where the work should go.
...
My OS sandbox auto-approves *simple* commands but prompts on anything it can't statically verify. To avoid drowning me in approval prompts:
...
- **Never add AI attribution to a commit.** No `Co-Authored-By:` trailers, no `🤖`, no "Generated with", no mention of Claude/Anthropic/any AI tool. Commits must read as if I wrote them myself.
...
- **Use Conventional Commits** (`<type>: <imperative, lowercase, no trailing period>`) per the `git-commit` skill.
