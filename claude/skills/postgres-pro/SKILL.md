...
When implementing PostgreSQL solutions, provide: the query with `EXPLAIN (ANALYZE, BUFFERS)` output and interpretation → index definitions with rationale and pre/post verification → configuration changes with before/after values → monitoring queries for ongoing health → a brief note on the performance impact.

## Related Skills

- **`security-and-hardening`** — SQL-injection prevention and data protection at the application boundary.
- **`debugging-and-error-recovery`** — root-causing slow queries and failures, which this skill's `EXPLAIN` analysis feeds.
