...
Run the prepare script first — it stamps today's date, builds the dated output path, and dumps
the diff plus orientation (status / commits / changed files, or the PR description) in one call:

```bash
sh ${CLAUDE_SKILL_DIR}/scripts/explain-diff.sh prepare [target] [--slug <short-name>]
```
...
- **Validate before finishing (required):** run the check and fix anything it reports as `FAIL`:
  ...
  ```bash
  sh ${CLAUDE_SKILL_DIR}/scripts/explain-diff.sh check <file>
  ```
...
## Sharing as an Artifact (opt-in only)

The local file above is always the default. Only publish as an Artifact if the user **explicitly
asks to share one** — and warn first: an Artifact uploads the content to claude.ai. It's private
by default, but it's still hosted off-machine and may be cached or indexed even if later deleted.
For confidential work code, keep it local.
