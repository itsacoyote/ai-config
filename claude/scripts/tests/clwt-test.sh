...
current_section=''
section() {
  if [ -n "${CLWT_TEST_STOP_AFTER:-}" ] && [ "$current_section" = "$CLWT_TEST_STOP_AFTER" ]; then
...
assert_pr_oid_matches_remote() {
  ...
  branch=$(sed -n 's/^headRefName=//p' "$CLWT_GH_PRS/$number")
  expected=$(sed -n 's/^headRefOid=//p' "$CLWT_GH_PRS/$number")
...
CLWT_TEST_SSH_LOG="$TMP/ssh.log"
: >"$CLWT_TEST_SSH_LOG"
export CLWT_TEST_SSH_LOG
...
launch_reset
: >"$CLWT_TEST_SSH_LOG"
new_unreachable_out=$(PATH="$SSH_FAIL_DIR:$PATH" clwt_in "$PRIMARY" new fix/ssh-unreachable-new 2>&1)
new_unreachable_rc=$?
unreachable_ssh_log=$(cat "$CLWT_TEST_SSH_LOG")
restore_origin
...
if [ ! -e "$MANAGED/fix-ssh-unreachable-new" ] && [ -z "$(launched pwd)" ]; then
  ok 'new neither creates a worktree nor launches claude when origin is unreachable'
else
...
launch_reset
lsremote_out=$(PATH="$FAILGIT_LSREMOTE:$PATH" clwt_in "$PRIMARY" new fix/lsremote-fails 2>&1)
lsremote_rc=$?
...
launch_reset
: >"$CLWT_TEST_SSH_LOG"
PATH="$SSH_PROXY_DIR:$PATH" clwt_in "$PRIMARY" new fix/ssh-over-ssh-new >/dev/null 2>&1
restore_origin

check_equals 'new over ssh creates the worktree and launches claude' \
  "$MANAGED/fix-ssh-over-ssh-new" "$(launched pwd)"
check_equals 'batch-mode ssh settings do not reach the launched claude session' \
  '<unset>' "$(launched GIT_SSH_COMMAND)"

proxy_lines=$(grep -c '^proxy: ' "$CLWT_TEST_SSH_LOG")
proxy_batch_lines=$(grep -c '^proxy: .*BatchMode=yes' "$CLWT_TEST_SSH_LOG")
...
launch_reset
: >"$CLWT_TEST_SSH_LOG"
branch_fail_out=$(PATH="$SSH_FAIL_DIR:$PATH" clwt_in "$PRIMARY" branch feat/ssh-unreachable-branch 2>&1)
restore_origin
...
launch_reset
: >"$CLWT_TEST_SSH_LOG"
PATH="$SSH_PROXY_DIR:$PATH" clwt_in "$PRIMARY" branch feat/ssh-proxy-branch >/dev/null 2>&1
restore_origin

check 'branch over ssh creates the worktree' \
  test -f "$MANAGED/feat-ssh-proxy-branch/ssh-proxy.txt"
if grep -q '^proxy: .*BatchMode=yes' "$CLWT_TEST_SSH_LOG"; then
...
launch_reset
: >"$CLWT_TEST_SSH_LOG"
PATH="$SSH_FAIL_DIR:$PATH" clwt_in "$PRIMARY" new fix/core-sshcommand >/dev/null 2>&1
(cd "$PRIMARY" && git config --unset core.sshCommand)
restore_origin

if grep -q '^core: .*BatchMode=yes' "$CLWT_TEST_SSH_LOG" && ! grep -q '^fail: ' "$CLWT_TEST_SSH_LOG"; then
...
launch_reset
: >"$CLWT_TEST_SSH_LOG"
GIT_SSH_COMMAND="$SSH_ENV_STUB" PATH="$SSH_FAIL_DIR:$PATH" clwt_in "$PRIMARY" new fix/env-sshcommand >/dev/null 2>&1
restore_origin

if grep -q '^env: .*BatchMode=yes' "$CLWT_TEST_SSH_LOG" && ! grep -q '^fail: ' "$CLWT_TEST_SSH_LOG"; then
...
launch_reset
: >"$CLWT_TEST_SSH_LOG"
GIT_SSH="$SSH_GITSSH_DIR/ssh" PATH="$SSH_FAIL_DIR:$PATH" clwt_in "$PRIMARY" new fix/gitssh-var >/dev/null 2>&1
restore_origin

if grep -q '^gitssh: .*BatchMode=yes' "$CLWT_TEST_SSH_LOG" && ! grep -q '^fail: ' "$CLWT_TEST_SSH_LOG"; then
...
# `--` reaches Claude and is never interpreted as a checkout reset.
...
check_output 'pr --force on a reused worktree reports verified reuse' \
  'reusing existing worktree' clwt pr 704 --force
...
launch_reset
check 'initial PR checkout succeeds before reuse refresh tests' clwt pr 801
...
check_equals 'initial PR checkout records its exact head marker' \
  "$(sed -n 's/^headRefOid=//p' "$CLWT_GH_PRS/801")" \
...
assert_pr_oid_matches_remote 801
launch_reset
check 'pr refreshes a clean reused worktree after a fast-forward' clwt pr 801
...
assert_pr_oid_matches_remote 801
launch_reset
check 'pr refreshes a rewritten head from its last verified commit' clwt pr 801
...
check_equals 'dirty refusal never launches claude' '' "$(launched pwd)"

pr_meta 803 feat/pr-reuse-local-commit false
clwt pr 803 >/dev/null 2>&1
...
check_equals 'local-commit refusal never launches claude' '' "$(launched pwd)"

pr_meta 804 feat/pr-reuse-fork-name true fork-one project
clwt pr 804 >/dev/null 2>&1
...
launch_reset
check_fails 'pr refuses a same-named reused branch from a different fork PR' clwt pr 805
...
check_equals 'wrong-fork refusal never launches claude' '' "$(launched pwd)"

pr_meta 806 feat/pr-reuse-unverified false
unverified_oid=$(sed -n 's/^headRefOid=//p' "$CLWT_GH_PRS/806")
...
launch_reset
check_fails 'pr refuses a markerless reused worktree with no tracking identity' clwt pr 806
check_equals 'unverified identity refusal never launches claude' '' "$(launched pwd)"

pr_meta 807 feat/pr-invalid-oid false
sed 's/^headRefOid=.*/headRefOid=not-an-object/' "$CLWT_GH_PRS/807" >"$CLWT_GH_PRS/807.tmp"
mv "$CLWT_GH_PRS/807.tmp" "$CLWT_GH_PRS/807"
launch_reset
check_fails 'pr refuses malformed headRefOid metadata' clwt pr 807
check_equals 'malformed OID refusal never launches claude' '' "$(launched pwd)"

pr_meta 808 feat/pr-oid-mismatch false
printf 'checkoutOidMismatch=true\n' >>"$CLWT_GH_PRS/808"
launch_reset
check_fails 'pr refuses launch when checkout HEAD differs from headRefOid' clwt pr 808
check_equals 'checkout OID mismatch never launches claude' '' "$(launched pwd)"
...
launch_reset
check_fails 'pr refuses when gh checks the right commit out under the wrong local branch' clwt pr 818
check 'wrong-local-branch checkout leaves no worktree behind' \
  test ! -e "$MANAGED/feat-pr-wrong-local-branch"
check_equals 'wrong-local-branch checkout never launches claude' '' "$(launched pwd)"
...
clwt pr 819 >/dev/null 2>&1
git -C "$PRIMARY" config --unset-all branch.feat/pr-reuse-partial-marker.worktree-pr-head
launch_reset
check_fails 'pr refuses a reused worktree with only half its identity markers' clwt pr 819
check_equals 'partial-marker refusal never launches claude' '' "$(launched pwd)"

pr_meta 820 feat/pr-reuse-malformed-marker false
clwt pr 820 >/dev/null 2>&1
git -C "$PRIMARY" config branch.feat/pr-reuse-malformed-marker.worktree-pr-head not-an-object
launch_reset
check_fails 'pr refuses a malformed last-verified head marker' clwt pr 820
check_equals 'malformed-marker refusal never launches claude' '' "$(launched pwd)"

pr_meta 821 feat/pr-invalid-url false
sed 's#^url=.*#url=not-a-canonical-pr-url#' "$CLWT_GH_PRS/821" >"$CLWT_GH_PRS/821.tmp"
mv "$CLWT_GH_PRS/821.tmp" "$CLWT_GH_PRS/821"
launch_reset
check_fails 'pr refuses malformed canonical URL metadata' clwt pr 821
check_equals 'malformed URL refusal never launches claude' '' "$(launched pwd)"

# A markerless worktree created by an older clwt can be migrated only from
...
legacy_ff_head=$(sed -n 's/^headRefOid=//p' "$CLWT_GH_PRS/809")
launch_reset
check 'markerless tracked PR worktree may refresh by fast-forward' clwt pr 809
...
launch_reset
check_fails 'markerless tracked PR worktree refuses a rewritten head' clwt pr 810
...
launch_reset
check_fails 'refresh refuses a target object at an exact ignored path' clwt pr 811
...
check_equals 'exact ignored-path collision never launches claude' '' "$(launched pwd)"
...
launch_reset
check_fails 'refresh refuses a target file replacing an ignored directory' clwt pr 812
...
check_equals 'file-ancestor collision never launches claude' '' "$(launched pwd)"
...
launch_reset
check_fails 'refresh refuses a target symlink replacing an ignored directory' clwt pr 823
...
check_equals 'symlink-ancestor collision never launches claude' '' "$(launched pwd)"

rm -rf "$PRIMARY/.env"
printf 'keep ignored blocking file\n' >"$PRIMARY/.env"
printf '.env\n' >"$PRIMARY/.worktreeinclude"
pr_meta 813 feat/pr-reuse-collision-directory false
clwt pr 813 >/dev/null 2>&1
rm -f "$PRIMARY/.worktreeinclude"
...
launch_reset
check_fails 'refresh refuses an ignored file blocking a target directory' clwt pr 813
...
primary_pr_oid=$(sed -n 's/^headRefOid=//p' "$CLWT_GH_PRS/814")
git -C "$PRIMARY" fetch -q origin "$primary_pr_oid"
git -C "$PRIMARY" checkout -q -b feat/pr-reuse-primary "$primary_pr_oid"
launch_reset
check_fails 'pr refuses reuse from the primary checkout' clwt pr 814
check_equals 'primary-checkout refusal never launches claude' '' "$(launched pwd)"
git -C "$PRIMARY" checkout -q main

pr_meta 815 feat/pr-reuse-unmanaged false
unmanaged_pr_oid=$(sed -n 's/^headRefOid=//p' "$CLWT_GH_PRS/815")
...
launch_reset
check_fails 'pr refuses reuse from an unmanaged worktree' clwt pr 815
check_equals 'unmanaged-worktree refusal never launches claude' '' "$(launched pwd)"
...
printf 'checkoutFails=true\n' >>"$CLWT_GH_PRS/816"
launch_reset
check_fails 'reused PR refuses when the disposable fetch fails' clwt pr 816
...
printf 'checkoutOidMismatch=true\n' >>"$CLWT_GH_PRS/817"
launch_reset
check_fails 'reused PR refuses when fetched HEAD differs from metadata' clwt pr 817
...
check_equals 'target-tree inspection failure never launches claude' '' "$(launched pwd)"
rm -f "$FAILGIT/git"

if git -C "$PRIMARY" worktree list --porcelain | grep -q 'clwt-pr-fetch'; then
...
# would launch a commit that is not the PR head. clwt must detect the mismatch,
