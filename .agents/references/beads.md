...
- **Never copy `.beads/` into a worktree.** Worktree helpers that copy ignored files
  rather than symlinking them can give each worktree its own database fork — writes then
  diverge and are lost. Exclude `.beads/**` from copied-file manifests and rely on the
  git-common-dir resolution above instead.
...
Before doing any workflow work, resolve `../../scripts/beads-preflight.sh` relative to the
active skill's directory, then execute that resolved absolute path. Agent Skills clients
expose the loaded skill's location; do not interpret the relative path from the repository
working directory. This works whether the complete `.agents/` library is project-local or
installed under `~/.agents/`. If the script exits non-zero, **stop** — do not proceed without
beads — and tell the user to run the `setup-beads` skill, then retry.
...
> `../../scripts/beads-preflight.sh` relative to this skill's directory and execute the
> resolved absolute path. If it exits non-zero, **stop** — do not proceed without beads — and
> tell the user to run the `setup-beads` skill, then retry.
