...
```
Diff scope: a1b2c3d4..e5f6a7b8 — changed files: .agents/references/diff-scope.md, .agents/agents/senior-review.md
```

## How a spawner computes it

A spawner runs **`diff-scope.sh`** from the library's `scripts/` directory — the single
source of the resolution logic — and copies the line it prints into the role dispatch. Do
not re-derive the merge-base inline. Resolve `../../scripts/diff-scope.sh` relative to the
active skill's directory and execute that resolved absolute path; do not interpret it from
the repository working directory. This works for project-local and global installations.
...
```text
Execute <resolved-skill-dir>/../../scripts/diff-scope.sh
...
```text
Execute <resolved-skill-dir>/../../scripts/diff-scope.sh --task
Execute <resolved-skill-dir>/../../scripts/diff-scope.sh --task <first-sha>
```
...
Resolve the script path first, then pass its `--range` output to `git diff`:

```bash
git diff "$(sh <resolved-diff-scope-script> --range)"
```
...
A review role needs permission to run `git diff` to consume a passed scope. Tool-policy
syntax and enforcement vary by harness, so declarations in role or skill metadata are not a
portable security boundary. The rev-parse / merge-base / symbolic-ref calls stay in the
spawner only — or in a role's self-derive fallback when its harness policy permits them.
...
Roles whose harness policy permits `git merge-base` and `git symbolic-ref` use the snippet
above. Roles limited to `git diff` may use the three-dot form instead:
