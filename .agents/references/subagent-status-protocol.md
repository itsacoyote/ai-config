# Isolated Worker Status Protocol

Shared vocabulary a **work worker** (for example, the `implementer` role) returns to the
**orchestrator** that spawned it (the main session, for example `autorun`). Workers have no
interactive-question or nested-worker capability. A worker therefore **never hangs and never
asks**: it finishes its turn with one of the statuses below, and the orchestrator (the only
party that talks to the human) decides what happens next.

This is the protocol for implementation workers. Review and verification roles keep the
verdict/result contract defined by their skill or role. The orchestrator maps those results
to the appropriate response logic.
...
| **DONE_WITH_CONCERNS** | Complete, but the worker flags something — a smell, a risk, a deviation from the plan, work it noticed but didn't do. | Read the concerns. If correctness-related, address before advancing. If observational, note it (file a follow-up issue) and continue. |
...
End the worker's turn with an explicit status line plus the specifics, e.g.:
...
- **Workers always return a status — never hang, never ask the human.** The orchestrator
...
- **A worker that can't decide picks `BLOCKED` and explains** — surfacing uncertainty is
