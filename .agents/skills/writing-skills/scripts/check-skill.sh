...
# AGENTS.md so they aren't eyeballed each time. It does NOT judge quality —
...
#         check-skill.sh --all                     check every .agents/skills/*/SKILL.md
...
check_one() {  # $1 = SKILL.md path; echoes results; returns 1 on hard fail
  ...
  for l in $links; do
    ...
    case "$target" in
      ...
      .agents/*) path="$ROOT/$target" ;;       # repo-root-relative
