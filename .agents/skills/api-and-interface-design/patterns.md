...
These are the API-design-specific applications of TypeScript's type system. For the general type-modeling techniques behind them (discriminated unions, `as const`, branded types, and deriving types from values), use the canonical portable `typescript-tips` skill—do not duplicate that guidance; build on it.
