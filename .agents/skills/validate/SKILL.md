...
- **Pi:** invoke read-only and verification roles through the absolute `.agents/scripts/run-pi-role.sh` generic runner path. It rejects implementation. Route every implementer fix through the sandbox-required `autorun/scripts/run-pi-implementer.sh` launcher with its canonical writer lease; if that external sandbox is unavailable, block rather than writing unsafely. Read-only workers may run independently, but QA and implementation remain serialized.
