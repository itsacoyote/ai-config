...
**Runtime mode** uses an available browser-automation capability—Chrome DevTools, Playwright, or an equivalent CLI/extension—to evaluate what markup alone cannot reveal: real focus order, computed contrast, breakpoint behavior, the accessibility tree, and interaction. Running the app and browser is evaluation, not a source-code change.

> Runtime requires the caller or harness adapter to provide browser automation and permission to start the app. Tool metadata in this skill is not a portable security boundary. When those capabilities are unavailable, use static mode; do not pretend runtime checks ran. Pi has no native MCP requirement, so use an installed CLI/extension or degrade gracefully.
...
- no browser automation capability is configured,
...
- Otherwise, review against **baseline WCAG 2.1 AA** using [`accessibility-checklist.md`](../../references/accessibility-checklist.md), and **note the absence** of a project standard in the verdict (an INFO-level finding — the project has no documented a11y bar).
...
Return every unresolved finding to the parent. The parent—not this read-only reviewer—files linked issues labelled `finding:design` according to [`beads.md`](../../references/beads.md).
...
- [`accessibility-checklist.md`](../../references/accessibility-checklist.md) — baseline WCAG 2.1 AA bar
- [`performance-checklist.md`](../../references/performance-checklist.md) — frontend performance checks
- `browser-testing-with-devtools` — drives the running app when a compatible browser capability is available; Playwright or another documented CLI/extension may be used instead
