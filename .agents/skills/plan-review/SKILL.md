...
- **Post-build code review** → use `validate` / [`senior-review`](../senior-review/SKILL.md). Those judge the diff after it's built.
...
The artifacts from `planning-and-task-breakdown`: the **architecture decisions**, the **file map** (each file's single responsibility + public interface), the **task breakdown** with acceptance criteria, the **dependency graph**, and the **risks** — checked against the **spec** it's supposed to satisfy.

Source them from beads: read the spec from the epic (`bd show <epic>`) and the tasks from its children. See [`beads.md`](../../references/beads.md) for the full model.
...
3. **Interfaces** — Per `api-and-interface-design`: are the public surfaces in the file map hard to misuse? Leaky abstractions, surfaces exposing internals, contracts between modules/tasks that don't line up.
...
Return every unresolved finding to the parent. The parent—not this read-only reviewer—files linked issues labelled `finding:plan` according to [`beads.md`](../../references/beads.md) and owns plan revisions and the beads lifecycle.
