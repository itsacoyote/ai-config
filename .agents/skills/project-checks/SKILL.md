---
name: project-checks
description: Use when an implementation task is complete or Validate is about to begin and the repository's mechanical format, lint, typecheck, spellcheck, and test gates need to run.
metadata:
...
These are not the same as the judgment review in `validate`
...
- **Before every commit** — the primary trigger. Run the checks as the last step before
  `git commit` (see `git-commit`) so you never commit something
...
Resolve `scripts/project-checks.sh` relative to this skill's directory, then execute that
absolute path from the repository root:

```text
<resolved-script>          # discover + run, fail-fast
<resolved-script> --list   # discover only — print the plan, run nothing
<resolved-script> -k       # keep going after failures
```
