...
Craft has **multiple user gates**, not one. When the harness has native image generation (for example Codex `image_gen`), the gate sequence before code is:
...
If the directory is empty (greenfield), don't pick a framework silently. Ask the user through the current harness's question capability, with sensible defaults framed by the brief:
...
If the harness has **native image generation**, this step is mandatory. **Stop and load [codex.md](codex.md)**. It covers palette generation, mock exploration, the approval loop, mock-fidelity inventory, and asset slicing via an `impeccable_asset_producer` isolated worker. Follow Steps A–F, then return here for Step 4. Harnesses without native image generation skip this branch; do not claim the capability exists.
...
- **Verify image URLs before referencing them.** Use any available image-search or web-fetch capability, including a documented CLI/extension fallback; guessed photo IDs ship as broken placeholders. Without verification, prefer fewer images you are confident about.
