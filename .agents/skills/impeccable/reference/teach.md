...
- **Both exist**: STOP and ask the user directly through the current harness's question capability. Ask which file to refresh. Skip the one the user doesn't want changed.
...
STOP and ask the user directly through the current harness's question capability. Ask only about what you couldn't infer from the codebase.
...
If the signal is genuinely split (e.g. a product with a big marketing landing), STOP and ask the user directly through the current harness's question capability. Ask which register describes the **primary** surface. The register can be overridden per task later, but PRODUCT.md carries one default.
...
**Critical: re-run the loader to refresh session context.** After writing PRODUCT.md, run `node <skill-dir>/scripts/load-context.mjs` one final time and let its full JSON output land in conversation. This ensures subsequent commands in this session use the freshly-written PRODUCT.md, not a stale earlier version.
...
Optionally stop and ask whether the user wants a brief PRODUCT.md summary appended to root `AGENTS.md` for portable agent reference. If an existing `CLAUDE.md` is active and the user explicitly wants Claude compatibility, offer to mirror the same short **Design Context** pointer there. Never modify either guidance file without explicit approval.
