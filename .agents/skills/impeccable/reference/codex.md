# Native Image Generation: Visual Direction & Asset Production

Load this file for `impeccable craft` only when the active harness provides native image generation (for example, Codex `image_gen` or an equivalent capability). Harnesses without that capability skip it. Never claim parity or ask the user to install an image service. This covers visual direction and production of raster assets the implementation will compose.
...
Use the active harness's native image-generation tool directly or through its documented image-generation skill. Do not ask the user to install anything.
...
Raster ingredients identified in Step E need clean production assets. Prefer a fresh, scoped `impeccable_asset_producer` isolated worker when the harness supports one. Follow [`isolated-worker-orchestration.md`](../../../references/isolated-worker-orchestration.md); for Pi, preload this complete reference and all required context into a fresh `pi --print` worker rather than assuming nested-agent support.

If isolated execution needs user approval, stop and ask:

```text
Asset production will work better as an isolated worker job. Should I start the Impeccable asset producer for this step?
```
...
Attach image generation capability to the worker when the harness supports it. Do **not** load image-generation reference material into the parent thread.

Inline asset production is allowed only if the user declines isolated execution, the harness cannot start an authorized worker, or the user explicitly asks for single-session mode.
