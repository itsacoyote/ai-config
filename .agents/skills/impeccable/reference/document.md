...
If a `DESIGN.md` already exists, **do not silently overwrite it**. Show the user the existing file, stop, and ask through the current harness's question capability whether to refresh, overwrite, or merge.
...
The following require creative input that cannot be auto-extracted. Group them into one user prompt:
...
4. **Refresh the session cache.** Run `node <skill-dir>/scripts/load-context.mjs` one final time so the newly-written DESIGN.md lands in conversation. Subsequent commands in this session will use the fresh version automatically without re-reading.
...
Group them into one user prompt. Options must be concrete.
...
3. Run `node <skill-dir>/scripts/load-context.mjs` once so the seed lands in conversation for the rest of the session.
