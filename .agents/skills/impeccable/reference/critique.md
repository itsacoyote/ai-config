...
CLI scan:
```bash
node <skill-dir>/scripts/detect.mjs --json [--fast] [target]
```
...
4. If mutation is available, start `node <skill-dir>/scripts/live-server.mjs --background`, present the browser if supported, label `[Human]`, scroll top, inject `http://localhost:PORT/detect.js`, wait 2-3 seconds, read `impeccable` console messages, then stop the live server.
...
**After presenting findings**, use targeted questions based on what was actually found. STOP and ask the user directly through the current harness's question capability. These answers will shape the action plan.
