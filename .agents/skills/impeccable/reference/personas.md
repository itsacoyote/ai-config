...
Resolve project-specific design context in this order:

1. A `## Design Context` section in root `AGENTS.md` (the portable source).
2. A `## Design Context` section in root `CLAUDE.md` when present (Claude compatibility).
3. The audience and brand sections in PRODUCT.md loaded by Impeccable.

When any of those sources contains real audience information, derive 1–2 additional personas:
...
**Profile**: [2-3 key characteristics derived from the selected design context]
