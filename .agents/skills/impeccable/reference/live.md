...
- **Cursor**: run the poll in the **foreground** (blocking shell, not a background terminal or isolated worker). Cursor background terminals and workers do not reliably resume chat with poll stdout.
- **Codex**: run the poll in the **foreground** (blocking shell, not a background task or isolated worker). Background exec sessions do not reliably surface poll stdout when events arrive.
- **Pi**: run the poll in the **foreground in the dedicated live session**. Do not spawn a `pi --print` worker for the long poll because its event loop must remain attached to the session handling browser edits.
...
```bash
node <skill-dir>/scripts/live.mjs
```
...
```
LOOP:
  node <skill-dir>/scripts/live-poll.mjs   # default long timeout; no --timeout=
...
```bash
node <skill-dir>/scripts/live-status.mjs
node <skill-dir>/scripts/live-resume.mjs --id SESSION_ID
node <skill-dir>/scripts/live-complete.mjs --id SESSION_ID
```
...
```bash
node <skill-dir>/scripts/live-wrap.mjs --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div" --text "TEXT_SNIPPET"
```
...
```bash
node <skill-dir>/scripts/live-poll.mjs --reply EVENT_ID done --file RELATIVE_PATH
```
...
```bash
node <skill-dir>/scripts/live-poll.mjs --reply EVENT_ID error "Short reason"
```
...
```bash
node <skill-dir>/scripts/live-server.mjs stop
```
...
```bash
node <skill-dir>/scripts/detect-csp.mjs
```
