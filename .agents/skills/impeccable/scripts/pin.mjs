 ...
 *
 * `pin audit` creates a lightweight explicit audit shortcut for Impeccable.
 * `unpin audit` removes that shortcut.
 *
 * The script discovers portable and harness-specific skill directories in the
 * project root and creates/removes the pin in all of them.
 */
...
function generatePinnedSkill(command, metadata) {
  const desc = metadata[command]?.description || `Use when explicitly invoking the Impeccable ${command} workflow.`;
...
function pin(command, projectRoot) {
  ...
  for (const skillsDir of harnessDirs) {
    ...
    if (harness === '.agents' || harness === '.codex') {
