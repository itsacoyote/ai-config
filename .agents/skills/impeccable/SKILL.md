---
name: impeccable
description: Use when improving any frontend interface — visual design, UX, accessibility, layout, typography, color, motion, or copy. Not for backend-only or non-UI tasks.
version: 3.1.1
...
Before any design work or file edits, resolve `<skill-dir>` to the directory containing this `SKILL.md`; every bundled command below uses that absolute location.
...
### Harness vocabulary in preserved references

Most bundled design references are mirrored byte-for-byte from Impeccable. Interpret their client-specific capability names portably:

- `AskUserQuestion` means ask through the current harness's interactive question capability, or use a plain conversational question when none exists.
- An image-search MCP means any available image-search, web-fetch, browser, CLI, or extension capability. Pi has no native MCP requirement; degrade to an installed CLI/extension or ask for a known asset.

These names document intent, not required tools or security boundaries.
...
```bash
node <skill-dir>/scripts/load-context.mjs
```
...
Apply to every design, both registers. Match implementation complexity to the aesthetic vision: maximalism needs elaborate code, minimalism needs precision. Interpret creatively. Vary across projects; never converge on the same choices. The active model is capable of extraordinary work. Don't hold back.
...
**Pin** creates a standalone explicit shortcut for an Impeccable subcommand. **Unpin** removes it. The script writes to every supported harness directory where Impeccable is already installed; Codex-compatible pins include explicit-only policy metadata.
