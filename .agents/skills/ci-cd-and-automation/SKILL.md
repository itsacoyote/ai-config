...
Fix the issue and verify locally before pushing again."
    │
    ▼
Implementation worker fixes → pushes → CI runs again
```

**Key patterns:**

```
Lint failure → implementation worker runs `npm run lint --fix` and commits
Type error  → implementation worker reads the error location and fixes the type
Test failure → implementation worker reproduces locally and finds the root cause (see `debugging-and-error-recovery`), fixes, and re-verifies
Build error → implementation worker checks config and dependencies
```
