---
name: senior-review
description: Use when a code change needs rigorous independent review for completeness, correctness, coherence, and YAGNI before it ships.
metadata:
...
- Structural smells per the baseline in [`code-smells.md`](../../references/code-smells.md) (Feature Envy, Data Clumps, Shotgun Surgery, …) — judgment calls, labelled as such; a documented repo standard overrides the baseline.
...
Return every unresolved finding to the parent. The parent—not this read-only reviewer—files linked issues labelled `finding:senior` according to [`beads.md`](../../references/beads.md).
