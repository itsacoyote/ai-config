---
name: sync
description: Use when explicitly preparing a local checkout for new feature work and it must be updated from the main branch without destructive git operations or automatic migrations.
metadata:
...
Explicitly invoked by the developer, this pre-feature ritual brings the local checkout up to date with `main` before new work begins. The mechanics live in a script; resolve `scripts/sync.sh` relative to this skill directory, run its absolute path, make the two human decisions with the developer, and relay its summary.
...
```bash
sh <skill-dir>/scripts/sync.sh --dry-run
```
...
```bash
sh <skill-dir>/scripts/sync.sh [--stash] [--install all|<csv>] [--main <branch>]
```
