...
Resolve `scripts/standup-gather.sh` relative to this skill directory and run its absolute path. It performs deterministic, **read-only** collection so you don't re-derive the commands. Pass the time window as its argument (default: last 24h):

```bash
sh <skill-dir>/scripts/standup-gather.sh                 # last 24h
sh <skill-dir>/scripts/standup-gather.sh 3d              # 24h / 3d / 2w
sh <skill-dir>/scripts/standup-gather.sh "since friday"  # or a phrase: "last week", "yesterday"
```
