...
Defaults drift between versions — confirm the current default before assuming.
The values below are starting points (sized here for a ~16GB RAM server),
not absolutes; the tuning *ratios and reasoning* are what carry over.

```sql
-- Memory settings
shared_buffers = 4GB           -- ~25% of RAM
effective_cache_size = 12GB    -- ~75% of RAM (planner hint, not an allocation)
work_mem = 64MB                -- Per sort/hash op, per connection — multiply out
maintenance_work_mem = 1GB     -- For VACUUM, CREATE INDEX

-- Checkpoint tuning (spread writes to avoid I/O spikes)
checkpoint_completion_target = 0.9
wal_buffers = 16MB

-- Query planner
random_page_cost = 1.1         -- Lower toward 1.0 for SSD; default assumes spinning disk
effective_io_concurrency = 200 -- Raise for SSD (concurrent prefetch)
