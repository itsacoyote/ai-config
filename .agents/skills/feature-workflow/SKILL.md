...
description: Use when starting or resuming a feature, or when you need the ordered Define → Research → Plan → Implement → Validate → Document map and its explicit handoffs.
metadata:
  category: workflow
---
...
This is portable on purpose — it travels with the complete `.agents/` library and complements the target project's own `AGENTS.md`.
...
| 2 | **Research** | `research` skill → dispatches neutral lens roles (`research-reuse`, `research-patterns`, `research-risks`; conditional `research-libraries`, ask-first `research-history`) when isolation is available | Findings: reuse, gaps, patterns, risks, architectural context — synthesized |
...
State flows through **beads** (required) — see [`beads.md`](../../references/beads.md) for the full model. Isolated work follows [`isolated-worker-orchestration.md`](../../references/isolated-worker-orchestration.md). There is no `.docs/` folder and no `context.yaml`; beads is the system of record.

## When NOT to use

Skip the full workflow for trivial work — a one-line fix does not need the whole pipeline. The workflow earns its keep on real features where a missed requirement or skipped review is expensive. Match the rigor to the change.
