...
This skill orchestrates from the **main session**, because Step 3 needs a fresh-context reviewer through the current harness's isolated-worker mechanism. Read [`isolated-worker-orchestration.md`](../../references/isolated-worker-orchestration.md) for the Claude, Codex, and Pi mappings.

- **Run it from the main session, not from inside an isolated worker.** Nested delegation is unavailable or deliberately disabled in the supported harness adapters.
- **If you find yourself inside an isolated worker:** surface that the doubt cycle must return to the main session. As a last resort, use degraded self-questioning: rewrite ARTIFACT + CONTRACT as a fresh self-prompt with a hard mental separator, then walk Steps 1–5. This is **not fresh-context review**, so label it degraded and prefer escalation whenever the user is reachable.
...
Spawn a fresh read-only reviewer using the harness mapping in `isolated-worker-orchestration.md` and hand it the adversarial prompt above. Use a general exploration worker for broad checks or a matching neutral role such as `senior-review` for engineering quality or `qa-review` for test coverage.

**The adversarial prompt above takes precedence over the reviewer's default response shape.** A role such as `senior-review` normally returns its own findings format; doubt-driven needs issues-only output. Include the adversarial framing explicitly. If the named role cannot be steered cleanly, use a generic fresh read-only worker.
