#!/usr/bin/env bash
# cwt-test.sh — self-contained test suite for codex/scripts/cwt
#
# Builds a throwaway world under a fake $HOME: a bare "remote", a primary clone,
# and stub `codex` / `gh` binaries on PATH that log how they were invoked. That
...
#
# Usage:  bash codex/scripts/tests/cwt-test.sh
...
# pwd -P'd, so on a symlinked home cwt disowned the worktrees it had just made.
...
# symlinked $HOME above, this means cwt must resolve the *whole* managed-root
...
export CWT_TEST_LOG="$TMP/launch.log"
: >"$CWT_TEST_LOG"

# Stub `codex`: records the working directory it inherited, the tracker-root env
...
#
# PR state per branch comes from $CWT_GH_STATES/<branch with / as ->. A missing
...
if [ "$1" = "pr" ] && [ "$2" = "view" ]; then
  case "$*" in
    *headRefName*)
      # `cwt pr` asking for a pull request's metadata by number.
      meta="$CWT_GH_PRS/$3"
      if [ ! -f "$meta" ]; then
      ...
      ;;
    *)
      # `cwt prune` asking for a branch's merge state.
      state_file="$CWT_GH_STATES/$(printf '%s' "$3" | tr '/' '-')"
...
if [ "$1" = "pr" ] && [ "$2" = "checkout" ]; then
  ...
  # cwt's pre-checkout decision, not gh's fork mechanics. The `+` forces the
...
STUB
chmod +x "$BIN/gh"
export CWT_GH_UNAVAILABLE="$TMP/gh-unavailable"
export CWT_GH_STATES="$TMP/gh-states"
export CWT_GH_PRS="$TMP/gh-prs"
mkdir -p "$CWT_GH_STATES" "$CWT_GH_PRS"
pr_state() { printf '%s\n' "$2" >"$CWT_GH_STATES/$(printf '%s' "$1" | tr '/' '-')"; }
# pr_meta <number> <head-ref> <is-cross-repository> [head-owner] [head-repo] —
# owner/repo default to the identity cwt derives from $REMOTE
...
# tracking ref cwt already knows about from an earlier session. Call this
...
# `cwt new` must base on the branch origin considers default *now*, not the one
...
# `cwt` is invoked from inside the primary clone unless a test says otherwise.
cwt_in() {
  local dir=$1
  shift
  (cd "$dir" && "$CWT" "$@")
...
# own copy rather than a call into cwt: a fixture guard that reused the code
...
section 'script'
check 'the cwt script exists and is executable' test -x "$CWT"
check 'the cwt script parses as valid bash' bash -n "$CWT"
if grep -q 'python' "$CWT" 2>/dev/null; then
  not_ok 'the cwt script has no python dependency'
else
  ok 'the cwt script has no python dependency'
fi
...
for sub in $SUBCOMMANDS; do
  cwt help 2>&1 | grep -qE "^ *$sub( |$)" || missing="$missing $sub"
done
if [ -z "$missing" ]; then
  ok 'cwt help lists all ten subcommands'
else
  not_ok "cwt help lists all ten subcommands (missing:$missing)"
fi

help_text=$(cwt help)
check_equals 'cwt --help prints the same usage as cwt help' "$help_text" "$(cwt --help)"
check_equals 'cwt -h prints the same usage as cwt help' "$help_text" "$(cwt -h)"
check_equals 'cwt new --help prints the usage' "$help_text" "$(cwt new --help)"
check_equals 'cwt remove --help prints the usage' "$help_text" "$(cwt remove --help)"
check_equals 'cwt prune --help prints the usage' "$help_text" "$(cwt prune --help)"

check_output 'an unknown subcommand is reported as unknown' \
  'unknown command' cwt definitely-not-a-command
check_fails 'an unknown subcommand exits non-zero' cwt definitely-not-a-command

# ------------------------------------------------------- identity and roots

section 'repo identity and roots'

# `cwt debug-roots` prints the resolved foundation values, one per line, so the
# suite can assert them without reaching into the script's internals.
roots() { cwt_in "$1" debug-roots 2>/dev/null; }
field() { printf '%s\n' "$1" | sed -n "s/^$2=//p"; }

R=$(roots "$PRIMARY")
check_equals 'cwt derives the owner from the origin remote' 'owner' "$(field "$R" owner)"
check_equals 'cwt derives the repo from the origin remote' 'project' "$(field "$R" repo)"
...
ssh_probe=$(cd "$PRIMARY" && git remote set-url origin 'git@github.com:someone/thing.git' \
  && "$CWT" debug-roots 2>/dev/null)
check_equals 'cwt derives owner and repo from an ssh origin remote' \
...
https_probe=$(cd "$PRIMARY" && git remote set-url origin 'https://github.com/someone/thing.git' \
  && "$CWT" debug-roots 2>/dev/null)
check_equals 'cwt derives owner and repo from an https origin remote' \
...
check_fails 'cwt exits non-zero when the origin remote is missing' \
  cwt_in "$TMP/no-remote" debug-roots
check_output 'cwt names the missing origin remote in its error' \
  'origin' cwt_in "$TMP/no-remote" debug-roots

# Not a repository at all.
mkdir -p "$TMP/not-a-repo"
check_fails 'cwt outside a git repository exits non-zero' \
  cwt_in "$TMP/not-a-repo" debug-roots
check_output 'cwt says it must be run inside a git repository' \
  'git repository' cwt_in "$TMP/not-a-repo" debug-roots
...
probe_remote() {
  (cd "$PRIMARY" && git remote set-url origin "$1" && "$CWT" debug-roots 2>&1)
}
...
# A managed worktree (created by hand here — cwt new arrives in task 3) and an
...
UNMANAGED=$(cd "$UNMANAGED" && pwd -P) # resolved, like every path cwt reports

check_output 'list shows a managed worktree by branch name' 'feat/listed' cwt list
check_output 'list shows the managed worktree path' "$MANAGED/feat-listed" cwt list
check_output 'list marks a worktree outside the managed root as unmanaged' 'unmanaged' cwt list
check_output 'list shows the unmanaged branch too' 'feat/stray' cwt list

bare_out=$(cwt 2>&1)
list_out=$(cwt list 2>&1)
check_equals 'cwt with no arguments prints the worktree list' "$list_out" "$bare_out"

check_fails 'list rejects extra arguments' cwt list nonsense

# The primary checkout is a worktree in git's eyes; it must not be offered as
# something cwt manages.
if cwt list 2>&1 | grep -qF "$PRIMARY "; then
...
check_output 'list reports plainly when there are no managed worktrees' \
  'No managed worktrees' cwt_in "$HOME/github/owner/lonely" list

# ------------------------------------------------------------------ launching

section 'launch primitive, root, and --yolo'

# The stub `codex` records the working directory and environment it was handed.
...
check_equals 'root launches codex in the primary checkout' "$PRIMARY" "$(launched pwd)"
check_equals 'root exports CWT_REPO_ROOT equal to the primary checkout' \
  "$PRIMARY" "$(launched CWT_REPO_ROOT)"

root_env=$(launched CWT_REPO_ROOT)
case $root_env in
  *.git) not_ok 'the exported CWT_REPO_ROOT does not end in .git' ;;
  *) ok 'the exported CWT_REPO_ROOT does not end in .git' ;;
esac
check_equals 'root passes no arguments to codex by default' '' "$(launched args)"
...
launch_reset
cwt_in "$MANAGED/feat-listed" root >/dev/null 2>&1
check_equals 'root launches in the primary checkout even when invoked from a worktree' \
  "$PRIMARY" "$(launched pwd)"
check_equals 'CWT_REPO_ROOT is correct when cwt is invoked from inside a worktree' \
  "$PRIMARY" "$(launched CWT_REPO_ROOT)"

# --yolo
launch_reset
cwt root --yolo >/dev/null 2>&1
check_equals '--yolo passes --yolo to codex' \
  '--yolo' "$(launched args)"

launch_reset
cwt root >/dev/null 2>&1
check_equals 'without --yolo no permission flag is passed to codex' '' "$(launched args)"

launch_reset
cwt root --yolo -- --model opus >/dev/null 2>&1
check_equals '--yolo composes with arguments after --' \
  '--yolo --model opus' "$(launched args)"

launch_reset
cwt root -- --model opus >/dev/null 2>&1
check_equals 'arguments after -- are passed through to codex' \
...
check_fails 'an unknown cwt-side flag is rejected rather than silently forwarded' \
  cwt root --yolol
check_output 'an unknown cwt-side flag is named in the error' 'yolol' cwt root --yolol
check_fails 'root rejects a positional argument' cwt root somebranch
...
launch_reset
cwt open feat/listed >/dev/null 2>&1
check_equals 'open launches codex with the worktree as its working directory' \
  "$MANAGED/feat-listed" "$(launched pwd)"
check_equals 'open exports CWT_REPO_ROOT set to the primary checkout' \
  "$PRIMARY" "$(launched CWT_REPO_ROOT)"

launch_reset
cwt open feat/listed --yolo >/dev/null 2>&1
check_equals '--yolo works on open as well as root' \
  '--yolo' "$(launched args)"

check_fails 'open refuses a worktree outside the managed root' cwt open feat/stray
check_output 'open explains that the worktree is outside the managed root' \
  'managed' cwt open feat/stray

check_fails 'open exits non-zero when the branch has no managed worktree' \
  cwt open feat/never-existed
check_output 'open names the branch it could not find' \
  'feat/never-existed' cwt open feat/never-existed

check_fails 'open requires a branch argument' cwt open
check_fails 'open rejects an invalid branch name' cwt open 'not a branch'
...
check_fails 'open refuses a symlinked worktree path inside the managed root' \
  cwt open feat/symlinked
rm -f "$MANAGED/feat-symlinked"

section 'new'

launch_reset
cwt new feat/alpha >/dev/null 2>&1
...
# proves cwt asked the remote what its default is *now* rather than trusting the
# cached ref.
check 'new bases the new branch on the current origin default branch' \
  test -f "$MANAGED/feat-alpha/stable.txt"

check_equals 'new launches codex in the worktree it created' \
  "$MANAGED/feat-alpha" "$(launched pwd)"

launch_reset
cwt new feat/beta --yolo >/dev/null 2>&1
...
check_fails 'new rejects a branch name without a type prefix' cwt new nomprefix
check_output 'new explains that a type prefix is required' 'feat/' cwt new noprefix
check_fails 'new rejects a branch name failing git check-ref-format' cwt new 'feat/bad..name'
check_fails 'new rejects a path-traversing branch name' cwt new '../evil'
check_fails 'new rejects a deeper path-traversing branch name' cwt new 'feat/../../evil'
check_fails 'new rejects a branch name containing whitespace' cwt new 'feat/a b'
...
check_fails 'new rejects a ref-valid name containing a shell metacharacter' \
  cwt new 'feat/a$b'
check_fails 'new requires a branch argument' cwt new
...
# `git worktree add`; cwt should catch it first.
...
check_fails 'new fails when the local branch already exists but is unchecked out' \
  cwt new feat/dormant
check_output 'new points at cwt branch when the local branch already exists' \
  'cwt branch' cwt new feat/dormant

section 'branch'

# An existing local branch with no worktree (created in the `new` section).
launch_reset
cwt branch feat/dormant >/dev/null 2>&1
check 'branch creates a managed worktree for an existing local branch' \
  test -d "$MANAGED/feat-dormant"
check_equals 'branch launches codex in that worktree' \
...
launch_reset
cwt branch feat/remote-only >/dev/null 2>&1
check 'branch checks out a branch that exists only on origin' \
  test -f "$MANAGED/feat-remote-only/remote-only.txt"
check_equals 'branch launches codex in the origin-only worktree' \
  "$MANAGED/feat-remote-only" "$(launched pwd)"

check_fails 'branch fails for a branch that exists nowhere' cwt branch feat/nonexistent
check_output 'branch names the branch it could not find' \
  'feat/nonexistent' cwt branch feat/nonexistent
check_fails 'branch requires a branch argument' cwt branch
check_fails 'branch rejects an invalid branch name' cwt branch 'feat/a b'
...
CWT_TEST_SSH_LOG="$TMP/ssh.log"
: >"$CWT_TEST_SSH_LOG"
export CWT_TEST_SSH_LOG
...
launch_reset
: >"$CWT_TEST_SSH_LOG"
new_unreachable_out=$(PATH="$SSH_FAIL_DIR:$PATH" cwt_in "$PRIMARY" new fix/ssh-unreachable-new 2>&1)
new_unreachable_rc=$?
unreachable_ssh_log=$(cat "$CWT_TEST_SSH_LOG")
restore_origin
...
if [ ! -e "$MANAGED/fix-ssh-unreachable-new" ] && [ -z "$(launched pwd)" ]; then
  ok 'new neither creates a worktree nor launches codex when origin is unreachable'
else
  not_ok 'new neither creates a worktree nor launches codex when origin is unreachable'
fi
...
launch_reset
lsremote_out=$(PATH="$FAILGIT_LSREMOTE:$PATH" cwt_in "$PRIMARY" new fix/lsremote-fails 2>&1)
lsremote_rc=$?
...
launch_reset
: >"$CWT_TEST_SSH_LOG"
PATH="$SSH_PROXY_DIR:$PATH" cwt_in "$PRIMARY" new fix/ssh-over-ssh-new >/dev/null 2>&1
restore_origin

check_equals 'new over ssh creates the worktree and launches codex' \
  "$MANAGED/fix-ssh-over-ssh-new" "$(launched pwd)"
check_equals 'batch-mode ssh settings do not reach the launched codex session' \
  '<unset>' "$(launched GIT_SSH_COMMAND)"

proxy_lines=$(grep -c '^proxy: ' "$CWT_TEST_SSH_LOG")
proxy_batch_lines=$(grep -c '^proxy: .*BatchMode=yes' "$CWT_TEST_SSH_LOG")
...
launch_reset
: >"$CWT_TEST_SSH_LOG"
branch_fail_out=$(PATH="$SSH_FAIL_DIR:$PATH" cwt_in "$PRIMARY" branch feat/ssh-unreachable-branch 2>&1)
restore_origin
...
launch_reset
: >"$CWT_TEST_SSH_LOG"
PATH="$SSH_PROXY_DIR:$PATH" cwt_in "$PRIMARY" branch feat/ssh-proxy-branch >/dev/null 2>&1
restore_origin

check 'branch over ssh creates the worktree' \
  test -f "$MANAGED/feat-ssh-proxy-branch/ssh-proxy.txt"
if grep -q '^proxy: .*BatchMode=yes' "$CWT_TEST_SSH_LOG"; then
...
launch_reset
: >"$CWT_TEST_SSH_LOG"
PATH="$SSH_FAIL_DIR:$PATH" cwt_in "$PRIMARY" new fix/core-sshcommand >/dev/null 2>&1
(cd "$PRIMARY" && git config --unset core.sshCommand)
restore_origin

if grep -q '^core: .*BatchMode=yes' "$CWT_TEST_SSH_LOG" && ! grep -q '^fail: ' "$CWT_TEST_SSH_LOG"; then
...
launch_reset
: >"$CWT_TEST_SSH_LOG"
GIT_SSH_COMMAND="$SSH_ENV_STUB" PATH="$SSH_FAIL_DIR:$PATH" cwt_in "$PRIMARY" new fix/env-sshcommand >/dev/null 2>&1
restore_origin

if grep -q '^env: .*BatchMode=yes' "$CWT_TEST_SSH_LOG" && ! grep -q '^fail: ' "$CWT_TEST_SSH_LOG"; then
...
launch_reset
: >"$CWT_TEST_SSH_LOG"
GIT_SSH="$SSH_GITSSH_DIR/ssh" PATH="$SSH_FAIL_DIR:$PATH" cwt_in "$PRIMARY" new fix/gitssh-var >/dev/null 2>&1
restore_origin

if grep -q '^gitssh: .*BatchMode=yes' "$CWT_TEST_SSH_LOG" && ! grep -q '^fail: ' "$CWT_TEST_SSH_LOG"; then
...
launch_reset
: >"$CWT_TEST_SSH_LOG"
GIT_SSH_COMMAND="$SSH_ENV_STUB" PATH="$SSH_FAIL_DIR:$PATH" cwt_in "$PRIMARY" new fix/precedence-env-over-core >/dev/null 2>&1
(cd "$PRIMARY" && git config --unset core.sshCommand)
restore_origin

if grep -q '^env: ' "$CWT_TEST_SSH_LOG" && ! grep -q '^core: ' "$CWT_TEST_SSH_LOG"; then
...
launch_reset
: >"$CWT_TEST_SSH_LOG"
GIT_SSH="$SSH_GITSSH_DIR/ssh" PATH="$SSH_FAIL_DIR:$PATH" cwt_in "$PRIMARY" new fix/precedence-core-over-gitssh >/dev/null 2>&1
(cd "$PRIMARY" && git config --unset core.sshCommand)
restore_origin

if grep -q '^core: ' "$CWT_TEST_SSH_LOG" && ! grep -q '^gitssh: ' "$CWT_TEST_SSH_LOG"; then
...
launch_reset
cwt branch feat/alpha >/dev/null 2>&1
...
# with "already checked out"; cwt should say something more useful.
...
check_fails 'branch refuses when the branch is checked out in the primary checkout' \
  cwt branch "$primary_branch"
check_output 'that refusal names the primary checkout' \
  "$PRIMARY" cwt branch "$primary_branch"
check_output 'that refusal suggests cwt root' \
  'cwt root' cwt branch "$primary_branch"
...
check_fails 'branch refuses when the branch is checked out in an unmanaged worktree' \
  cwt branch feat/stray
check_output 'that refusal names the unmanaged path' "$UNMANAGED" cwt branch feat/stray
check_output 'that refusal says cwt does not manage it' 'manage' cwt branch feat/stray

# The three cases must be distinguishable, not one generic message.
msg_primary=$(cwt branch "$primary_branch" 2>&1 || true)
msg_unmanaged=$(cwt branch feat/stray 2>&1 || true)
...
launch_reset
cwt new feat/no-include >/dev/null 2>&1
...
launch_reset
cwt new feat/empty-include >/dev/null 2>&1
...
launch_reset
copy_out=$(cwt new feat/copied 2>&1)
...
launch_reset
beads_out=$(cwt new feat/beads-guard 2>&1)
...
# --exclude-from resolves relative to the current directory, and cwt is often
# run from inside a worktree. The pattern file and the copy source must both come
# from the primary checkout regardless of where cwt was invoked.
cat >"$PRIMARY/.worktreeinclude" <<'PATTERNS'
.env
PATTERNS
launch_reset
cwt_in "$MANAGED/feat-alpha" new feat/from-inside >/dev/null 2>&1
check 'worktreeinclude is read from the primary checkout when cwt runs inside a worktree' \
...
launch_reset
cwt branch feat/copy-on-branch >/dev/null 2>&1
...
PATTERNS
  ...
  launch_reset
  copyfail_out=$(cwt new feat/copy-fails 2>&1 || true)

  check_equals 'a failed copy does not launch codex' '' "$(launched pwd)"
...
# before the guard existed. `cwt pr` makes this reachable from a fork.
(
  cd "$TMP/seed"
...
PATTERNS
launch_reset
symlink_out=$(cwt new feat/symlink-escape 2>&1 || true)
check_equals 'a symlinked copy destination does not launch codex' '' "$(launched pwd)"
...
PATTERNS
launch_reset
nested_out=$(cwt new feat/nested-escape 2>&1 || true)
check_equals 'a symlinked intermediate directory does not launch codex' '' "$(launched pwd)"
...
PATTERNS
launch_reset
dir_out=$(cwt new feat/dir-escape 2>&1 || true)
check_equals 'a directory at the copy destination does not launch codex' '' "$(launched pwd)"
...
launch_reset
cwt new feat/removable >/dev/null 2>&1
check 'the worktree to remove exists first' test -d "$MANAGED/feat-removable"
check 'remove deletes a clean managed worktree' cwt remove feat/removable
...
# --delete-branch
launch_reset
cwt new feat/disposable >/dev/null 2>&1
check 'remove --delete-branch succeeds' cwt remove feat/disposable --delete-branch
...
# worktree cwt creates becomes unremovable — and the destruction must be named.
cat >"$PRIMARY/.worktreeinclude" <<'PATTERNS'
.env
PATTERNS
printf 'SECRET=1\n' >"$PRIMARY/.env"
launch_reset
cwt new feat/has-ignored >/dev/null 2>&1
...
remove_out=$(cwt remove feat/has-ignored 2>&1)
...
launch_reset
cwt new feat/self-remove >/dev/null 2>&1
check_fails 'remove refuses the worktree containing the caller working directory' \
  cwt_in "$MANAGED/feat-self-remove" remove feat/self-remove
...
check_output 'the self-removal refusal explains itself' \
  'standing in' cwt_in "$MANAGED/feat-self-remove" remove feat/self-remove
check 'removing it from elsewhere still works' cwt remove feat/self-remove

# Guards inherited from the managed-root contract.
check_fails 'remove refuses a worktree outside the managed root' cwt remove feat/stray
check 'the unmanaged worktree survives' test -d "$UNMANAGED"
check_fails 'remove fails for a branch with no worktree' cwt remove feat/never-existed
check_fails 'remove requires a branch name' cwt remove
check_fails 'remove rejects an unknown flag' cwt remove feat/listed --nope
...
# that call fails and the rest of cwt still works.
FAILGIT="$TMP/failgit"
mkdir -p "$FAILGIT"
make_failing_git() {
...
# function, so `env PATH=… cwt_in …` fails for that reason alone and any
# check_fails around it passes vacuously.
cwt_with_failing_git() {
  PATH="$FAILGIT:$PATH" cwt_in "$PRIMARY" "$@"
}

launch_reset
cwt new feat/failing-git >/dev/null 2>&1
printf 'SECRET=1\n' >"$MANAGED/feat-failing-git/.env"

make_failing_git 'ls-files'
out=$(PATH="$FAILGIT:$PATH" cwt remove feat/failing-git 2>&1 || true)
...
check_fails 'remove exits non-zero when git cannot list ignored files' \
  cwt_with_failing_git remove feat/failing-git
...
launch_reset
cwt new feat/merged-self >/dev/null 2>&1
pr_state feat/merged-self MERGED
cwt_in "$MANAGED/feat-merged-self" prune --yes >/dev/null 2>&1
check 'prune never removes the worktree containing the caller working directory' \
  test -d "$MANAGED/feat-merged-self"
check 'prune removes it once the caller is elsewhere' cwt prune --yes
...
touch "$CWT_GH_UNAVAILABLE"
launch_reset
cwt new feat/merged-b >/dev/null 2>&1
pr_state feat/merged-b MERGED
check_fails 'prune exits non-zero when gh is unavailable' cwt prune
check_output 'prune says why it cannot determine merge state' 'prune needs gh' cwt prune
check_fails 'prune --yes also exits non-zero when gh is unavailable' cwt prune --yes
...
cwt_without_gh() { PATH="$NOGH" cwt_in "$PRIMARY" "$@"; }

check_fails 'prune exits non-zero when gh is not on PATH at all' \
  cwt_without_gh prune
check_output 'prune distinguishes gh missing from gh unauthenticated' \
  'not on PATH' cwt_without_gh prune
check_output 'pr also reports gh missing from PATH' \
  'not on PATH' cwt_without_gh pr 101

check_fails 'prune rejects a positional argument' cwt prune something
check_fails 'prune rejects an unknown flag' cwt prune --force
...
check 'install succeeds when ~/.local/bin does not exist' cwt install
check 'install creates the ~/.local/bin directory' test -d "$LOCAL_BIN"
check 'install creates a symlink at ~/.local/bin/cwt' test -L "$LOCAL_BIN/cwt"
check_equals 'the installed symlink points at the repo script' \
  "$CWT" "$(readlink "$LOCAL_BIN/cwt" 2>/dev/null)"

# Idempotence.
check 'install is idempotent when the correct symlink already exists' cwt install
check_output 'install says it is already installed on a repeat run' 'already' cwt install

# Refuses to clobber a real file.
reset_install
mkdir -p "$LOCAL_BIN"
printf 'not a symlink\n' >"$LOCAL_BIN/cwt"
check_fails 'install refuses to clobber an existing non-symlink file' cwt install
check_output 'install explains why it refused to clobber' 'not a symlink' cwt install
check 'the pre-existing file survives a refused install' \
  grep -q 'not a symlink' "$LOCAL_BIN/cwt"

# Repoints a symlink that aims somewhere else.
rm -f "$LOCAL_BIN/cwt"
ln -s "$TMP/some-other-cwt" "$LOCAL_BIN/cwt"
check 'install repoints a symlink that aims elsewhere' cwt install
check_equals 'the repointed symlink now targets the repo script' \
  "$CWT" "$(readlink "$LOCAL_BIN/cwt" 2>/dev/null)"

# Warns when the install directory is not on PATH.
reset_install
off_path_out=$(PATH="$BIN:/usr/bin:/bin" cwt install 2>&1)
...
reset_install
cwt install >/dev/null 2>&1
via_symlink=$("$LOCAL_BIN/cwt" install 2>&1)
...
check 'install through the installed symlink succeeds' \
  test -L "$LOCAL_BIN/cwt"

# install must work outside a git repository — it has nothing to do with a repo.
check 'install works outside a git repository' cwt_in "$TMP/not-a-repo" install
...
check 'pr launches codex in that worktree' test -n "$(launched pwd)"
...
# checkout` fails after the worktree already exists. Found by running `cwt pr`
...
check_fails 'pr exits non-zero when gh cannot check the pull request out' cwt pr 303
...
check_fails 'a retry after a failed checkout still fails cleanly' cwt pr 303

check_fails 'pr exits non-zero when the pull request number does not exist' cwt pr 999
check_output 'pr names the number it could not resolve' '999' cwt pr 999
check_fails 'pr requires a pull request number' cwt pr
check_fails 'pr rejects a non-numeric argument' cwt pr not-a-number

touch "$CWT_GH_UNAVAILABLE"
check_fails 'pr exits non-zero when gh is unavailable' cwt pr 101
check_output 'pr says it needs gh' 'pr needs gh' cwt pr 101
rm -f "$CWT_GH_UNAVAILABLE"
rm -f "$PRIMARY/.worktreeinclude"
...
launch_reset
check_fails 'a failed --force checkout leaves no worktree behind' cwt pr 303 --force
...
check_fails 'branch rejects --force as an unknown option' cwt branch --force
check_fails 'root rejects --force as an unknown option' cwt root --force
...
# cwt CPU-bound for minutes before launch. This smaller fixture still crosses
# the hard CPU budget with that nested loop, while remaining quick to construct.
pr_meta 826 feat/pr-reuse-many-ignored false
cwt pr 826 >/dev/null 2>&1
exclude_file="$PRIMARY/.git/info/exclude"
printf '.cwt-perf/\n' >>"$exclude_file"
mkdir -p "$MANAGED/feat-pr-reuse-many-ignored/.cwt-perf"
perf_i=0
while [ "$perf_i" -lt 8000 ]; do
  printf 'ignored local content\n' \
    >"$MANAGED/feat-pr-reuse-many-ignored/.cwt-perf/file-$perf_i"
  perf_i=$((perf_i + 1))
done
...
check 'reused PR launch stays within its time budget with thousands of ignored files' \
  within_scan_budget "$PRIMARY" "$CWT" pr 826
check_equals 'large ignored-file refresh still launches codex' \
...
# The "equal" scenario: a tracking ref cwt already fetched once (established
...
launch_reset
check 'pr auto-resets a leftover branch strictly behind origin' cwt pr 706
...
launch_reset
check_fails 'pr refuses a leftover branch carrying a local-only commit' cwt pr 707
check_output 'that refusal suggests cwt pr N --force' 'cwt pr 707 --force' cwt pr 707
...
launch_reset
missing_ref_out=$(cwt pr 711 2>&1)
missing_ref_rc=$?
...
launch_reset
check 'pr with no leftover branch is unaffected by the probe' cwt pr 712
...
launch_reset
force_hint_out=$(cwt pr 713 --force 2>&1)
force_hint_rc=$?
...
launch_reset
cross_empty_out=$(cwt pr 714 2>&1)
cross_empty_rc=$?
...
launch_reset
adversarial_out=$(cwt pr 715 2>&1)
adversarial_rc=$?
...
# exactly the way `cwt pr` does: from inside a fresh detached worktree of
...
# stub directly — ahead of and independent from any cwt-side behavior — so later
...
# zsh does not autoload Bash completions. The documented setup uses bashcompinit,
# which must be able to source this file and register its public function. Keep
# zsh optional: cwt itself requires Bash, not zsh.
if command -v zsh >/dev/null 2>&1; then
  if zsh -f -c '
    autoload -Uz compinit && compinit -u -d "$1"
    autoload -Uz bashcompinit && bashcompinit
    source "$2"
    whence -w _cwt | grep -q function
  ' cwt-test "$TMP/zcompdump" "$COMPLETION" >/dev/null 2>&1; then
    ok 'the completion script registers cwt through zsh bashcompinit'
  else
    not_ok 'the completion script registers cwt through zsh bashcompinit'
  fi
else
  skip 'zsh bashcompinit smoke test (zsh unavailable)'
fi
...
if [ -f "$COMPLETION" ]; then
  ...
  # COMPREPLY is set by _cwt in the current shell, so the call cannot be
  # subshelled — the caller cd's to the directory it wants completions from.
  complete_for() {
    COMP_WORDS=("$@")
    COMP_CWORD=$((${#COMP_WORDS[@]} - 1))
    COMPREPLY=()
    _cwt >/dev/null 2>&1 || true
    printf '%s\n' "${COMPREPLY[@]-}"
  }
  ...
  subs=$(complete_for cwt '')
  missing=''
  for sub in new branch open pr root list remove prune install help; do
    printf '%s\n' "$subs" | grep -qx "$sub" || missing="$missing $sub"
  done
  if [ -z "$missing" ]; then
    ok 'completion offers all ten subcommands for a bare cwt'
  else
    not_ok "completion offers all ten subcommands for a bare cwt (missing:$missing)"
  fi

  filtered=$(complete_for cwt 'pr')
  ...
  branches=$(complete_for cwt branch '')
  ...
  removes=$(complete_for cwt remove '')
  ...
  hostile_completions=$(complete_for cwt branch 'feat/x')
  ...
  rm -f "$RCE_MARKER"
  managed_hostile=$(complete_for cwt open 'feat/x')
  if [ -e "$RCE_MARKER" ]; then
  ...
  yolo=$(complete_for cwt new feat/x '--')
  ...
  del=$(complete_for cwt remove feat/alpha '--')
  ...
  pr_flags=$(complete_for cwt pr 701 '--')
  ...
  # is absent from it.
  branch_flags=$(complete_for cwt branch feat/alpha '--')
  ...
  outside_branches=$(complete_for cwt open '')
...
# These assert against repository artifacts rather than the sandbox. cwt is a
# developer-facing Codex CLI, not a shared Agent Skill or native Codex setting.
section 'codex integration'

CODEX_README="$REPO_ROOT/codex/README.md"
...
# A prior CLI section landed inside a Markdown fence. Pin both edited documents:
# balanced fences are necessary, and the cwt heading must also be outside them.
for document in "$CODEX_README" "$ROOT_README"; do
  fences=$(grep -c '^```' "$document")
  if [ $((fences % 2)) -eq 0 ]; then
    ok "$(basename "$document") code fences are balanced"
  else
    not_ok "$(basename "$document") code fences are balanced (found $fences)"
  fi
done
if python3 - "$CODEX_README" "$ROOT_README" <<'PY'
import sys
for path in sys.argv[1:]:
    ...
    for line in open(path):
        if line.startswith('```'):
            inside = not inside
        elif 'cwt' in line and line.startswith('#') and inside:
            sys.exit(1)
sys.exit(0)
PY
then
  ok 'cwt headings are real headings, not fenced examples'
else
  not_ok 'cwt headings are real headings, not fenced examples'
fi

readme_missing=''
for sub in new branch open pr root list remove prune install help; do
  grep -qE "cwt $sub" "$CODEX_README" || readme_missing="$readme_missing $sub"
done
if [ -z "$readme_missing" ]; then
  ok 'the Codex README documents all ten subcommands'
else
  not_ok "the Codex README documents all ten subcommands (missing:$readme_missing)"
fi

check 'the Codex README documents the managed root layout' grep -qF '.worktrees' "$CODEX_README"
check 'the Codex README documents CWT_REPO_ROOT' grep -qF 'CWT_REPO_ROOT' "$CODEX_README"
check 'the Codex README documents native --yolo' grep -qF -- '--yolo' "$CODEX_README"
...
check 'the Codex README documents the cwt binary install path' grep -qF '~/.local/bin/cwt' "$CODEX_README"
check 'the Codex README documents the cwt completion install path' \
  grep -qF 'bash-completion/completions/cwt' "$CODEX_README"
check 'the Codex README documents stock macOS Bash 3.2 support' grep -qF 'Bash 3.2' "$CODEX_README"
check 'the Codex README documents zsh bashcompinit setup' grep -qF 'bashcompinit' "$CODEX_README"
check 'the Codex README documents the shared clwt and cwt managed root' \
  grep -qiE 'clwt.*cwt|cwt.*clwt' "$CODEX_README"
if tr '\n' ' ' <"$CODEX_README" | grep -qiE 'run by the developer|run it from your shell|human-run'; then
  ok 'the Codex README says launching subcommands are run by the developer'
else
  not_ok 'the Codex README says launching subcommands are run by the developer'
fi

check 'ADR 0011 exists' test -f "$ADR"
check 'ADR 0011 records the independent Codex port' grep -qiF 'independent' "$ADR"
check 'ADR 0011 records the cd plus exec choice over native -C' \
  grep -qiE 'cd.*exec|-C|--cd' "$ADR"
